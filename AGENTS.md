...
- Reject mutable statuses, current roles, relationships, affiliations, and cumulative statistics by default, but allow historically settled slices when deterministic provenance shows the historical answer is fixed.
...
- Record query and harvesting problems during runs so they can be reviewed and fixed later.

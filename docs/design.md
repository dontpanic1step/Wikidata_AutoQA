...
Terminology note: in the Route 3 table/infobox path, older references to LLM rewrite in the main generation path mean the generation model reading the selected table plus page context and producing a table-grounded question. The public Route 3 control for that model is `--generation-model`. The optional KELM-style post-generation rewrite pass is separate, off by default, and controlled by `--enable-kelm-rewrite` plus `--kelm-rewrite-model`; those KELM rewrite parameters do not choose the Route 3 table/infobox generation model.

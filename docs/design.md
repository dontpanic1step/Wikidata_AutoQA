...
Before prompting the model, Route 3 ranks extracted tables. Prefer article tables with useful subject/list context, tables with multiple structured rows, and row values that are not repeated in non-table prose, but do not penalize a table merely for being an infobox. The table extractor expands `rowspan` and `colspan` into a rectangular grid, combines multi-row headers into Markdown column labels, preserves blank cells, and sends Markdown table evidence to the model; the older header-to-row-dictionary parser is disabled. Numeric/comparable-value bonuses such as capacity/rank/count/date/votes fields are kept in code behind disabled switches for possible reuse, but they should not affect current page or table grading. Penalize oversized prose-like tables, placeholder/mutable tables such as live standings, and tables whose row values are already easy to recover from article text. Default early table filter modes drop unsuitable tables before any generation prompt: `no_picture_heavy_tables` rejects any wikitable with an image-bearing cell and rejects infoboxes whose image-bearing cells cover more than one quarter of table cells; `no_incomplete_tables` rejects tables containing markers such as `unlisted`, `incomplete`, `unknown`, `approx.`, `approximate`, `approximately`, `citation needed`, or `citing needed`; `not_number_dominant` rejects tables with markers such as `thousand`, `million`, `billion`, or `trillion`, common measurement unit markers, comma-separated numbers, decimals, no-comma numbers between 1000 and 1500 or above 2040, or table content whose alphabetic characters do not exceed 50% of non-whitespace characters; `no_social_science_research` rejects tables whose normalized text contains markers such as `census`, `survey`, `demographic`, `self reported`, `ancestry group`, `ethinic group`, `ethinicity`, `population`, or `language speaker`. A configurable `min_table_score` cutoff drops ranked tables before first-paragraph alias/context extraction and before the Route 3 generation prompt; the default cutoff is `0.0`. If no table survives the score cutoff, live-scope filter, and enabled table filter modes, the page is rejected before any generation call. By default, `llm_choose_table` is disabled and only the single top-ranked surviving table is passed to the small model with table-choice prompt text omitted. When `llm_choose_table` is enabled, the top three surviving ranked tables are passed with table-choice instructions. Store the ranking criteria, scores, cutoff, filter modes, matched markers, and cutoff/filter decisions as metadata so table choice can be audited.

For large Route 3 recipes, use big-batch mode so transient table-search discovery failures are retried instead of silently ending a segment, the stream batch size matches the MediaWiki search page size, compact rejected records avoid multi-gigabyte artifacts, and existing segment summaries are only reused when the segment actually used its requested page budget.

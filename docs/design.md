...
The durable boundaries are the allocation record, Wikipedia page archive, external-call records, terminal page-attempt ledger, and derived projections. Page preparation and deterministic validation are safe to recompute within the same attempt and have no independent checkpoints. All5 uses stable answer-type slot keys and commits all terminal slot outcomes atomically in one page attempt. A typed slot-level DDG or OpenRouter infrastructure failure during attempt001 makes the whole page eligible for attempt002 without publishing partial slot outcomes. During attempt002, exhaustion rejects only the failing slot and the worker continues the remaining slots before the atomic page commit. Deterministic candidate failures after slot creation reject only their slot, including persisted-response failures. A persisted-response failure in the shared page-generation call rejects the page. Unexpected Python or persistence exceptions propagate and leave the page attempt uncommitted. Attempt002 reuses matching external-call results by stable logical call key and request hash. Each independently published artifact uses a same-directory unique temporary file followed by close and `os.replace`; local stages do not form a generic hash graph or lock registry.
...
`scripts/convert_batch_evaluation_for_verification_agent.py` is a post-evaluation interoperability adapter. It accepts an input directory and a new output directory, scans only top-level `*.jsonl` files, and selects files by the current protected prediction/judging record schema: `id`, `question`, `answer`, and object-valued `predictions[]` items. Each qualifying source file is converted independently to a same-named file; nonconforming JSONL is skipped with an audited reason. The adapter does not participate in generation, prediction, judging, filtering, review, or finalization, and it does not alter either protected evaluation script.

Each source `predictions[]` item becomes one response row with the primary `model_jsonl` fields read by Project Verification Agent's real entry point, `runs/run_simpleqa_entry.py`: `original_index`, `query`, `model`, and `response`. Stable dataset-level question identity is preferred over response/model/repeat-specific IDs, and rows are clustered by stable `(question_id, query)` within each output file. The agent groups equal `(id, query)` rows into one sample's model outputs, then its retrieval workers and evidence-pool registry share evidence by exact query text within that run. The retained `reference_answer` is source metadata only and is not used to verify response claims.

The adapter is lossless at the JSON-value level. Each emitted response row stores the unchanged source record fields other than `predictions`, the unchanged selected prediction object, its original index and count, and source-file/line provenance. Predictions that cannot become agent rows because of missing identity/question data, an empty prediction list or response, or an unresolved model are stored with the same exact values and provenance under `unconverted_items` in `conversion_summary.json`; they are never silently dropped or converted into verification claims. Ordering emitted and unconverted items by source file, source line, and prediction index reconstructs every conforming input record. A qualifying file with no usable responses produces no empty agent JSONL and is listed as skipped. The summary also records converted/skipped files, counts, paths, and reasons.

"""Run Route 1 QID-first multi-hop join generation through shared filters."""
...
from wikidata_simpleqa.concurrency import (
    SemaphoreWrappedClient,
    build_shared_pipeline_concurrency,
)
...
from wikidata_simpleqa.generators import WikidataMultiHopJoinGenerator
...
def parse_args() -> argparse.Namespace:
    """Parse Route 1 multi-hop runner arguments."""
    ...
    parser.add_argument("--state", type=Path, default=ROOT / "outputs" / "route1_multihop_join_state.json")
    parser.add_argument("--output", type=Path, default=ROOT / "outputs" / "route1_multihop_join_accepted.jsonl")
    parser.add_argument(
        "--rejected-output",
        type=Path,
        default=ROOT / "outputs" / "route1_multihop_join_rejected.jsonl",
    )
    parser.add_argument("--summary-output", type=Path, default=ROOT / "outputs" / "route1_multihop_join_summary.json")
    parser.add_argument("--final-output", type=Path, default=ROOT / "outputs" / "route1_multihop_join_final.jsonl")
    parser.add_argument(
        "--final-summary-output",
        type=Path,
        default=ROOT / "outputs" / "route1_multihop_join_final_summary.json",
    )
    return parser.parse_args()


def main() -> int:
    """Run the Route 1 multi-hop join pipeline."""
    args = parse_args()
    ...
    settings = Settings(
        ...
        enabled_routes=(ROUTE1_MULTIHOP_JOIN_ROUTE,),
        proxy=proxy,
    ...
    if args.reset_state:
        state = Route1QidSeedState(path=args.state)
        state.save()
    else:
        state = Route1QidSeedState.load(args.state)
    ...
    templates = get_route1_multihop_join_templates(args.template_keys or None)
    generator = WikidataMultiHopJoinGenerator()
    ...
    for candidate in generated_candidates:
        if candidate.source_candidate is None:
            continue
        unit = seed_unit_from_candidate(candidate.source_candidate)
        seed_units.append(unit)
        candidate_by_seed_key[unit.state_key] = candidate
    remaining_limit = _remaining_after_endpoint(args.record_limit, endpoint_resume.final_decision_count) if args.start_from_endpoint else args.record_limit
    ...
    summary = {
        "route": ROUTE1_MULTIHOP_JOIN_ROUTE,
        ...
        "rerun_pool_failure_reasons_after_run": {
            key: state.failure_reasons.get(key, "")
            for key in state.rerun_pool
        },
    ...
    *,
    ...
    try:
        ...
        if result.accepted:
            with commit_lock:
                for index, record in enumerate(result.accepted):
                    record["id"] = f"route1_multihop_join_{len(state.accepted_keys) + index + 1:06d}"
        ...
        if result.rejected:
            ...
            if _should_rerun_route1_rejection(result.rejected[0]):
                with commit_lock:
...
def _build_second_stage_model_panel(settings: Settings, semaphore) -> list[ModelPanelMember]:
    ...
    return [
        ModelPanelMember(
            name=member.name,
            client=SemaphoreWrappedClient(member.client, semaphore, {"complete_text"}),
        )
...
def _write_final_selection(args: argparse.Namespace) -> tuple[list[dict], dict]:
    ...
    final_records, summary = select_final_records(
        records,
        target_count=max(0, int(args.final_target)),
        domain_key="domain",
    )
    for index, record in enumerate(final_records, start=1):
        record["id"] = f"route1_multihop_join_final_{index:06d}"
    ...
    args.final_summary_output.write_text(
...
def _endpoint_seed_keys(records: list[dict]) -> list[str]:
    """Return non-empty Route 1 QID seed keys from endpoint records."""
    return [key for key in (seed_key_from_record(record) for record in records) if key]
...
def _should_rerun_route1_rejection(record: dict) -> bool:
    """Return whether a Route 1 rejection is transient and worth rerunning."""
...
def _attach_record_metadata(record: dict, *, seed_key: str, args: argparse.Namespace) -> None:
    """Attach Route 1 run metadata to an accepted or rejected record."""
    metadata = record.setdefault("source_metadata", {})
    if isinstance(metadata, dict):
        metadata["route1_qid_seed_key"] = seed_key

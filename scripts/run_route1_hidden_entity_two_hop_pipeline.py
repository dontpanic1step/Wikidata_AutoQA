"""Disabled legacy Route 1 hidden-entity two-hop runner.

Use scripts/run_route4_two_hop_pipeline.py for the maintained Route 4 route.
"""
...
from wikidata_simpleqa.route1_hidden_entity import (
    ROUTE1_HIDDEN_ENTITY_TWO_HOP_ROUTE,
    hidden_entity_seed_key_from_record,
    hidden_entity_seed_unit_from_candidate,
)
...
def parse_args() -> argparse.Namespace:
    """Parse Route 1 hidden-entity runner arguments."""
    ...
    parser.add_argument("--state", type=Path, default=ROOT / "outputs" / "route1_hidden_entity_two_hop_state.json")
    parser.add_argument("--output", type=Path, default=ROOT / "outputs" / "route1_hidden_entity_two_hop_accepted.jsonl")
    parser.add_argument(
        "--rejected-output",
        type=Path,
        default=ROOT / "outputs" / "route1_hidden_entity_two_hop_rejected.jsonl",
    )
    parser.add_argument(
        "--summary-output",
        type=Path,
        default=ROOT / "outputs" / "route1_hidden_entity_two_hop_summary.json",
    )
    parser.add_argument(
        "--final-output",
        type=Path,
        default=ROOT / "outputs" / "route1_hidden_entity_two_hop_final.jsonl",
    )
    parser.add_argument(
        "--final-summary-output",
        type=Path,
        default=ROOT / "outputs" / "route1_hidden_entity_two_hop_final_summary.json",
    )
    return parser.parse_args()


def main() -> int:
    """Report that the legacy Route 1 hidden-entity runner is disabled."""
    args = parse_args()
    _ = args
    print(
        "route1_wikidata_hidden_entity_two_hop is disabled and will not be updated. "
        "Use scripts/run_route4_two_hop_pipeline.py with route4_wikidata_two_hop.",
        file=sys.stderr,
    )
    return 2
    ...
    settings = Settings(
        ...
        enabled_routes=(ROUTE1_HIDDEN_ENTITY_TWO_HOP_ROUTE,),
        proxy=proxy,
    ...
    for candidate in generated_candidates:
        if candidate.source_candidate is None:
            continue
        unit = hidden_entity_seed_unit_from_candidate(candidate.source_candidate)
    ...
    summary = {
        "route": ROUTE1_HIDDEN_ENTITY_TWO_HOP_ROUTE,
        ...
        if result.accepted:
            with commit_lock:
                for index, record in enumerate(result.accepted):
                    record["id"] = f"route1_hidden_entity_two_hop_{len(state.accepted_keys) + index + 1:06d}"
...
def _write_final_selection(args: argparse.Namespace) -> tuple[list[dict], dict]:
    ...
    for index, record in enumerate(final_records, start=1):
        record["id"] = f"route1_hidden_entity_two_hop_final_{index:06d}"
...
def _endpoint_seed_keys(records: list[dict]) -> list[str]:
    ...
    return [key for key in (hidden_entity_seed_key_from_record(record) for record in records) if key]
...
def _attach_record_metadata(record: dict, *, seed_key: str, args: argparse.Namespace) -> None:
    ...
    if isinstance(metadata, dict):
        metadata["route1_hidden_entity_seed_key"] = seed_key

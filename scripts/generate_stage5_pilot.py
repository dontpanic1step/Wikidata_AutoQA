"""Generate a Stage 5 pilot with one example per active template."""
...
def main() -> int:
    ...
    for template in get_active_templates():
        ...
        rejected_tmp_path = ROOT / "outputs" / f"tmp_{template.domain}_rejected.jsonl"
        resume_manifest_path = ROOT / "outputs" / f"tmp_{template.domain}_resume.json"
    ...
    accepted_path = ROOT / "outputs" / "stage5_pilot_accepted.jsonl"
    rejected_path = ROOT / "outputs" / "stage5_pilot_rejected.jsonl"
    summary_path = ROOT / "outputs" / "stage5_pilot_summary.json"

    accepted = _renumber_ids(accepted, "wikidata_verified_pilot")
